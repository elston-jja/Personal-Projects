...
class Game:
    ...
    def update(self):
        ...
        if self.lives <= 0:
            self.lose()
        elif self.lives > 0 and self.score == 40:
            self.win()
    ...
    def win(self):
        ...
        #// Here put code when people win the game
        print("You Win")
        self.end = True
...
class Ball(pygame.sprite.Sprite):
    ...
    def check_collisions(self):
        ...
        if player.rect.x  < self.rect.x < player.rect.x + 65:
            if player.rect.y -15 < self.rect.y < player.rect.y + 15:
...
class Paddle(pygame.sprite.Sprite):
    ...
    def move(self,direction):
        ...
        if self.offscreen:
            ...
            # Stop the paddle from going beyond screen borders
            # This is the default method, and can be toggled off
...
while not done:
    ...
    for event in pygame.event.get():
        ...
        # Get cordinates for mouse
        elif event.type == pygame.MOUSEBUTTONDOWN:
            print (pygame.mouse.get_pos())

        # Keep paddle move velocity constant in loop
    ...
    # Check game variable to stop
    if brickbreaker.end:
            done = True

    # set the background to color "bg"                
    screen.fill(bg)
    # Update all sprites and draw to screen
    all_sprites_list.update()
    all_sprites_list.draw(screen)
    # Update Game Info at top left
    brickbreaker.update()
    # Limit FPS and redraw screen
    clock.tick(60)
    pygame.display.flip()

# If pygame loop is done, quit
pygame.quit()


# BUGS
# - Multiple bricks break at the same time
# - Sometimes ball misses paddle (pygame issue ?)
